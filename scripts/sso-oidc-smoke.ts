 ...
 * buildOidcConfiguration, mounts it the way main.ts does (`app.use('/oidc',
 * provider.callback())`), and drives a full Authorization Code + PKCE sign-in through
 * HTTP -- completing the login and consent interactions exactly as InteractionController
 * does -- then exchanges the code for tokens and verifies the id_token.
 *
 * This is the gap the other suites left. `smoke:sso` exercises the authentication
 * *logic* (SsoAuthService) one layer below the HTTP surface; nothing booted the provider
 * or walked a real code flow, so nothing exercised `buildOidcConfiguration` itself. On
 * its first run this test caught two config bugs that made SSO fail for every relying
 * party in production -- a missing `id_token_signed_response_alg` and a missing
 * `subjectTypes: ['pairwise']` -- neither visible without a booted provider.
 *
 * Hermetic: an in-memory store and a locally generated OIDC key, no database, no network.
 * A stub PlatformService exposes only the members buildOidcConfiguration reads; the real
 * PlatformService wiring is covered by the unit-level suites.
 */
...
// The provider's configured issuer. oidc-provider uses this verbatim for the `iss`
// claim and the discovery `issuer` field, independent of the bind port (endpoint URLs
// in discovery are built from the request Host, so they still carry the real port).
const ISSUER = 'http://127.0.0.1/oidc';
...
class CookieJar {
  ...
  store(headers: Record<string, string | string[] | undefined>) {
...
/**
 * Follow redirects until the response points at the relying party's redirect_uri (the
 * end of the flow) or stops redirecting. Interaction routes complete themselves
 * server-side and redirect on, so the client only has to keep following.
 */
async function followUntilRedirectUri(start: HttpResponse, base: string, jar: CookieJar): Promise<HttpResponse> {
  let res = start;
  let hops = 0;
  while (res.location && hops++ < 15) {
    ...
    if (next.startsWith(REDIRECT_URI)) return res;
    res = await req('GET', next, jar);
  }
  if (process.env.DEBUG_OIDC) {
    console.error(`  [final] ${res.status} ${res.body.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').slice(0, 300)}`);
  }
  return res;
}
...
async function main() {
  ...
  check('pairwise subject type is enabled', ((config as any).subjectTypes ?? []).includes('pairwise'));
  ...
  const ended = await followUntilRedirectUri(await req('GET', authUrl, jar), base, jar);
  check('the flow ends by redirecting to the RP callback', (ended.location ?? '').startsWith(REDIRECT_URI));
  const cbUrl = new URL(ended.location ?? 'http://invalid');
  ...
  const ended2 = await followUntilRedirectUri(
    await req(
      'GET',
      `${base}/oidc/auth?client_id=${CLIENT_ID}&response_type=code&scope=${encodeURIComponent('openid profile')}` +
        `&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&code_challenge=${challenge2}&code_challenge_method=S256&state=x`,
      jar2,
    ),
    base,
    jar2,
  );
  const code2 = new URL(ended2.location ?? 'http://invalid').searchParams.get('code');
  const badExchange = await req('POST', `${base}/oidc/token`, jar2, {
    headers: { authorization: `Basic ${basic}` },
    body: `grant_type=authorization_code&code=${code2}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&code_verifier=${b64url(randomBytes(32))}`,
  });
  check('a wrong PKCE verifier is rejected (400)', badExchange.status === 400, `status ${badExchange.status}`);
  ...
  const publishedJwks = JSON.parse(jwksRes.body);
  check('the published JWKS exposes only the PUBLIC key (no private d)', !('d' in publishedJwks.keys[0]));

  const jwks = createLocalJWKSet(publishedJwks);
